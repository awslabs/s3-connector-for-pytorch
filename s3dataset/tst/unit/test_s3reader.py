...
def test_s3reader_creation(object_info, get_stream):
    s3reader = S3Reader(TEST_BUCKET, TEST_KEY, object_info, get_stream)
    assert s3reader
...
def test_s3reader_prefetch(stream):
    s3reader = S3Reader(TEST_BUCKET, TEST_KEY, None, lambda: stream)
...
def test_s3reader_seek(stream_and_positions: Tuple[List[bytes], List[int]]):
    ...
    s3reader = S3Reader(TEST_BUCKET, TEST_KEY, None, lambda: iter(stream))
...
def test_seeks_end():
    s3reader = S3Reader(TEST_BUCKET, TEST_KEY, None, lambda: iter([]))
    s3reader._size = 10
