...
@pytest.mark.parametrize(
    "datasetLambda",
    [
        lambda: S3MapDataset.from_objects([], region=None),
        lambda: S3MapDataset.from_objects([], region=123),
        lambda: S3MapDataset.from_prefix("s3://bucket/prefix", region=None),
        lambda: S3MapDataset.from_prefix("s3://bucket/prefix", region=123),
    ],
)
def test_dataset_creation_fails_without_region(
    datasetLambda: Callable[[], S3MapDataset]
):
