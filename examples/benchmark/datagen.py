...
class ImageGenerator(DataGenerator):
    ...
    def generate(self, index: int, destdir: Path):
        img = Image.fromarray(np.random.randint(0, high=256, size=(self.height, self.width, 3), dtype=np.uint8))
...
class Utils:
    ...
    @staticmethod
    def batch_files(files: list[Path], size_threshold: float) -> Generator[list[Path], None, None]:
        group: list[Path] = []
        total = 0
        for file in files:
    ...
    @staticmethod
    def parse_resolution(ctx: click.Context, param: str, value: str) -> tuple[int, int] | None:
...
@click.option("-n", "--num-samples", type=pr.Float, default='1k', help="Number of samples to generate.")
@click.option("--resolution", callback=Utils.parse_resolution, default='496x387',
              help="Resolution written in 'widthxheight' format")
@click.option("--shard-size", callback=Utils.parse_hr_bytes,
...
@click.option("--s3-bucket", type=str,
              ...
              required=True)
@click.option("--s3-prefix", type=str,
...
@click.option("--region", default='us-east-1', type=str, help="Region where the S3 bucket is hosted.")
def synthesize_dataset(num_samples: float, resolution: tuple[int, int], shard_size: float,
                       s3_bucket: str, s3_prefix: str, region: str):
    """
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        with parallel_config(backend="multiprocessing", n_jobs=cpu_count()):
            s = int(num_samples)
            Parallel()(delayed(generator.generate)(i, tempdir) for i in tqdm(range(s), desc="Generating data"))
            if shard_size:
                files: list[Path] = [(tempdir / file) for file in os.listdir(tempdir)]
                Parallel()(delayed(Utils.tar_files)(f"train_shard_{batch_idx}", files=file_batch, workdir=tempdir)
                           for (batch_idx, file_batch) in enumerate(Utils.batch_files(files, shard_size)))
        # upload to s3
        with parallel_config(backend="threading", n_jobs=cpu_count()):
            ...
            Parallel()(delayed(Utils.upload_to_s3)(region=region, file=file, bucket=s3_bucket, prefix=disambiguator)
        ...
        click.echo(f"Configure your experiment by setting the entry:\n\tdataset: {disambiguator}")
        click.echo("Alternatively, you can run specify it on the cmd-line when running the benchmark like so:")
